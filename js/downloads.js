...
export function showNotification(message) {
    ...
    notifEl.innerHTML = `<div style="display: flex; align-items: start;">${message}</div>`;
...
export function addDownloadTask(trackId, track, filename, api) {
    ...
    taskEl.innerHTML = `
        <div style="display: flex; align-items: start; gap: 0.75rem;">
            <img src="${api.getCoverUrl(track.album?.cover)}" style="width: 40px; height: 40px; border-radius: 4px; flex-shrink: 0;">
            ...
            <button class="download-cancel" style="background: transparent; border: none; color: var(--muted-foreground); cursor: pointer; padding: 4px; border-radius: 4px; transition: all 0.2s;">${SVG_CLOSE}</button>
        </div>
    `;
...
// Helper for single track downloads (same as before)
...
// Streaming generator for client-zip
async function* mapTracksToFiles(tracks, folderName, api, quality, lyricsManager, notification, totalTracks) {
    for (let i = 0; i < tracks.length; i++) {
        const track = tracks[i];
        const filename = buildTrackFilename(track, quality);
        const trackTitle = getTrackTitle(track);
        updateBulkDownloadProgress(notification, i + 1, totalTracks, trackTitle);
...
// Unified streaming ZIP downloader
async function streamZipDownload(filesGenerator, filename, notification) {
    updateBulkDownloadProgress(notification, 0, 0, 'Starting download...');

    try {
        const zipStream = downloadZip(filesGenerator).body;
        let writable;

        // 1. Try File System Access API (Chrome/Edge/Opera/Vivaldi)
        ...
        // Pipe the ZIP stream to the file writer
        if (writable) {
        ...
        completeBulkDownload(notification, false, 'Download failed');
    }
}
...
export async function downloadAlbumAsZip(album, tracks, api, quality, lyricsManager = null) {
    ...
    const filesGenerator = mapTracksToFiles(tracks, folderName, api, quality, lyricsManager, notification, tracks.length);
    
    await streamZipDownload(filesGenerator, folderName, notification);
}
...
export async function downloadPlaylistAsZip(playlist, tracks, api, quality, lyricsManager = null) {
    ...
    // Inject cover if available (needs to be first yielded item if we want it at top, but order doesn't strictly matter for ZIP)
    // We can wrap the generator to add cover.
    async function* generatorWithCover() {
        ...
        yield* mapTracksToFiles(tracks, folderName, api, quality, lyricsManager, notification, tracks.length);
    }

    await streamZipDownload(generatorWithCover(), folderName, notification);
}
...
export async function downloadDiscography(artist, api, quality, lyricsManager = null) {
    ...
    async function* discographyGenerator() {
        for (let i = 0; i < allReleases.length; i++) {
            const album = allReleases[i];
            updateBulkDownloadProgress(notification, i, allReleases.length, album.title);
            try {
                ...
                yield* mapTracksToFiles(tracks, fullFolderPath, api, quality, lyricsManager, notification, 0); // Pass 0 as total to avoid messing up progress bar text? 
                // Actually, mapTracksToFiles updates the text. We might want to customize progress for discography.
                // But for now, simple reuse is fine, it will just flicker "Track X/Y" while "Discography" title remains.
            } catch (err) {
                console.error(`Failed to process album ${album.title}`, err);
            }
        }
    }

    await streamZipDownload(discographyGenerator(), rootFolder, notification);
}
...
function createBulkDownloadNotification(type, name, totalItems) {
    ...
    notifEl.innerHTML = `
        <div style="display: flex; align-items: start; gap: 0.75rem;">
            <div style="flex: 1; min-width: 0;">
                <div style="font-weight: 600; font-size: 0.95rem; margin-bottom: 0.25rem;">Downloading ${typeLabel}</div>
...
function completeBulkDownload(notifEl, success = true, message = null) {
    ...
    if (success) {
        ...
        statusEl.style.color = '#10b981';
        setTimeout(() => { notifEl.style.animation = 'slideOut 0.3s ease'; setTimeout(() => notifEl.remove(), 300); }, 3000);
    } else {
        progressFill.style.background = '#ef4444';
        statusEl.textContent = message || '✗ Download failed';
        statusEl.style.color = '#ef4444';
        setTimeout(() => { notifEl.style.animation = 'slideOut 0.3s ease'; setTimeout(() => notifEl.remove(), 300); }, 5000);
    }
}

export async function downloadTrackWithMetadata(track, quality, api, lyricsManager = null, abortController = null) {
    if (!track) { alert('No track is currently playing'); return; }
    const filename = buildTrackFilename(track, quality);
    const controller = abortController || new AbortController();
    try {
        const { taskEl, taskAbortController } = addDownloadTask(track.id, track, filename, api);
        await api.downloadTrack(track.id, quality, filename, {
            signal: controller.signal,
            track: track,
            onProgress: (progress) => { updateDownloadProgress(track.id, progress); }
        });
        completeDownloadTask(track.id, true);
        if (lyricsManager && lyricsSettings.shouldDownloadLyrics()) {
            try {
                const lyricsData = await lyricsManager.fetchLyrics(track.id, track);
                if (lyricsData) lyricsManager.downloadLRC(lyricsData, track);
            } catch (error) { console.log('Could not download lyrics for track'); }
        }
    } catch (error) {
        if (error.name !== 'AbortError') {
            const errorMsg = error.message === RATE_LIMIT_ERROR_MESSAGE ? error.message : 'Download failed. Please try again.';
